    ...
    return (
        ...
        {isHome ? "" : 
            <div className="breadcrumb-wrapper">
                ...
                {i+1 == hlinks.length 
                ? 
                <span  key={bread.id} className="breadcrumb-label">{bread.loc}</span> 
                : 
                <><a  key={bread.id} href={bread.path} className="breadcrumb-label when-link">{bread.loc}</a> <span className="delim">/</span></>
                }
                </>
                ))}
            </div>
